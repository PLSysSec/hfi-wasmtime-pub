...
fn store(engine: &Engine) -> Store<WasiCtx> {
    let wasi = WasiCtxBuilder::new().arg("data/html").expect("couldn't set program name in argv").build();
    //wasi.push_arg("20");
...
fn bench_html(
    ...
    let mut group = c.benchmark_group(format!(
        "html: {} instances, batching: {}, slot_size: {} MiB",
        instance_slot_count,
        batching,
        slot_size >> 20
    ));
...
fn bench_instantiation_html(c: &mut Criterion) {
    // for file in std::fs::read_dir("benches/instantiation").unwrap() {
        // let path = file.unwrap().path();
        let mut path = PathBuf::new();
        path.push("benches/modules/html_template.wasm");
        ...
        for &(batching, instance_slot_count, slot_size, guard) in &[
            ...
            bench_html(c, &path, instance_slot_count, batching, slot_size, guard);
        }
    // }
}
...
criterion_group!{
    ...
    targets = bench_instantiation_html
}

// criterion_group!(benches_html, bench_instantiation_html);
criterion_main!(benches_html);
