...
fn store(engine: &Engine) -> Store<WasiCtx> {
    let wasi = WasiCtxBuilder::new().arg("jpeg").expect("couldn't set program name in argv").arg("20").expect("Couldn't set argv?").build();
    //wasi.push_arg("20");
...
fn bench_jpeg(
    ...
    let mut group = c.benchmark_group(format!(
        "jpeg: {} instances, batching: {}, slot_size: {} MiB",
        instance_slot_count,
        batching,
        slot_size >> 20
    ));
...
fn bench_instantiation_jpeg(c: &mut Criterion) {
    // for file in std::fs::read_dir("benches/instantiation").unwrap() {
        // let path = file.unwrap().path();
        let mut path = PathBuf::new();
        path.push("benches/modules/jpeg_resize_c.wasm");
        ...
        for &(batching, instance_slot_count, slot_size, guard) in &[
            ...
            bench_jpeg(c, &path, instance_slot_count, batching, slot_size, guard);
        }
    // }
}

criterion_group!(benches_jpeg, bench_instantiation_jpeg);
criterion_main!(benches_jpeg);
