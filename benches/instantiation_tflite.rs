...
fn store(engine: &Engine) -> Store<WasiCtx> {
    let wasi = WasiCtxBuilder::new().arg("tflite").expect("couldn't set program name in argv").build();
    //wasi.push_arg("20");
...
fn bench_tflite(
    ...
    let mut group = c.benchmark_group(format!(
        "tflite: {} instances, batching: {}, slot_size: {} MiB",
        instance_slot_count,
        batching,
        slot_size >> 20
    ));
...
fn bench_instantiation_tflite(c: &mut Criterion) {
    // for file in std::fs::read_dir("benches/instantiation").unwrap() {
        // let path = file.unwrap().path();
        let mut path = PathBuf::new();
        path.push("benches/modules/tflite.wasm");
        ...
        for &(batching, instance_slot_count, slot_size, guard) in &[
            ...
            bench_tflite(c, &path, instance_slot_count, batching, slot_size, guard);
        }
    // }
}

criterion_group!(benches_tflite, bench_instantiation_tflite);
criterion_main!(benches_tflite);
