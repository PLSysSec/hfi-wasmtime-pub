...
fn store(engine: &Engine) -> Store<WasiCtx> {
    let xml = include_str!("data/test_msg.txt");
    let wasi = WasiCtxBuilder::new().arg("xml").expect("couldn't set program name in argv").arg(xml).expect("Couldn't load xml into argv?").build();
    //wasi.push_arg("20");
...
fn bench_xml(
    ...
    let mut group = c.benchmark_group(format!(
        "xml: {} instances, batching: {}, slot_size: {} MiB",
        instance_slot_count,
        batching,
        slot_size >> 20
    ));
...
fn bench_instantiation_xml(c: &mut Criterion) {
    // for file in std::fs::read_dir("benches/instantiation").unwrap() {
        // let path = file.unwrap().path();
        let mut path = PathBuf::new();
        path.push("benches/modules/xml_to_json.wasm");
        ...
        for &(batching, instance_slot_count, slot_size, guard) in &[
            ...
            bench_xml(c, &path, instance_slot_count, batching, slot_size, guard);
        }
    // }
}

criterion_group!(benches_xml, bench_instantiation_xml);
criterion_main!(benches_xml);
