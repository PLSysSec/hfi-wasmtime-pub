...
fn store(engine: &Engine) -> Store<WasiCtx> {
    let wasi = WasiCtxBuilder::new().arg("fib").expect("couldn't set program name in argv").arg("4").expect("Couldn't set argv?").build();
    //wasi.push_arg("20");
...
fn bench_fib(
    ...
    let mut group = c.benchmark_group(format!(
        "fib: {} instances, batching: {}, slot_size: {} MiB",
        instance_slot_count,
        batching,
        slot_size >> 20
    ));
...
fn bench_instantiation_fib(c: &mut Criterion) {
    // for file in std::fs::read_dir("benches/instantiation").unwrap() {
        // let path = file.unwrap().path();
        let mut path = PathBuf::new();
        path.push("benches/modules/fib_c.wasm");
        ...
        for &(batching, instance_slot_count, slot_size, guard) in &[
            ...
            bench_fib(c, &path, instance_slot_count, batching, slot_size, guard);
        }
    // }
}

criterion_group!(benches_fib, bench_instantiation_fib);
criterion_main!(benches_fib);
